...
const fieldsThatDoNotNeedToBeLocallySaved = [
  'datesortie',
  'heuresortie',
  'reason',
]

const overrideFormDataWithLocalStorageValues = (field) =>
  fieldsThatDoNotNeedToBeLocallySaved.includes(field.key)
    ? field
    : {
        ...field,
        value: localStorageWrapper().getItem(field.key),
      }
...
export function createForm () {
  ...
  const formFirstPart = formData
    ...
    .map(overrideFormDataWithLocalStorageValues)
    .map((field, index) => {
